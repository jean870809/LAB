    ...
    for (int exp_times = 0; exp_times < EXP_TIME; exp_times++)
    {
        ...
        while (T > 0.0001)
        {
            for (int i = 0; i < stepsize; i++)
            {
                ...
                if (neighbor_right > x_max)
                {
                    neighbor_right = x_max;
                }
                double neighbor_left = x - step[i];
                if (neighbor_left < x_min)
                {
                    neighbor_left = x_min;
                }
                ...
                while ((f(neighbor_right, 1) < f(x, 1)) || (f(neighbor_left, 1) < f(x, 1))) //接受解-如果左或右鄰居小於目前解
                {
                    ...
                    if (f(neighbor_left, 1) < f(x, 1)) //左鄰居<目前解 往左鄰居靠近 接受左鄰居
                    {
                    ...
                    else if (f(neighbor_right, 1) < f(x, 1)) //右鄰居<目前解 往右鄰居靠近 接受右鄰居
                    {
                    ...
                    if (r < exp(-(f(neighbor_right, 1) - f(x, 1)) / T))
                    {
                        x = neighbor_right;
                        neighbor_right = x + 1;
                        neighbor_left = x - 1;
                    }
                    else if (r < exp(-(f(neighbor_left, 1) - f(x, 1)) / T))
                    {
                        x = neighbor_left;
                        neighbor_right = x + 1;
                        neighbor_left = x - 1;

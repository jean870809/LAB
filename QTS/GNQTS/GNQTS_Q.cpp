...
int main()
{
    ...
    for (int y = 2009; y <= 2019; y++)
    {
        for (int m = 1; m <= 12; m++)
        {
            ...
            string input_name = "D:/LAB/c++/QTS/DJI_30/Q2M/train_";
            string input_format = " Q1).csv";
            stringstream ss_input;
            if (m + 2 > 12)
            {
                ss_input << input_name << y << "_" << setw(2) << setfill('0') << m << "~" << y + 1 << "_" << setw(2) << setfill('0') << m - 10 << "(" << y << input_format;
            }
            else
            {
                ss_input << input_name << y << "_" << setw(2) << setfill('0') << m << "-" << setw(2) << setfill('0')<< m + 2 << "(" << y << input_format;
            }

            input_csv = ss_input.str();

            string output_name = "train_result/Q2M/train_";
            string output_format = " Q1)_result.csv";
            stringstream ss_output;
            if (m + 2 > 12)
            {
                ss_output << output_name << y << "_" << setw(2) << setfill('0') << m << "~" << y + 1 << "_" << setw(2) << setfill('0') << m - 10 << "(" << y << output_format;
            }
            else
            {
                ss_output << output_name << y << "_" << setw(2) << setfill('0') << m << "-" << m + 2 << "(" << y << output_format;
            }

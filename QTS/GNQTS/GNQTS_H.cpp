...
int main()
{
    ...
    for (int y = 2009; y <= 2019; y++)
    {
        for (int q = 1; q <= 4; q++)
        {
            ...
            string input_name = "D:/LAB/c++/QTS/DJI_30/H2Q/train_";
            string input_format = " Q1).csv";
            stringstream ss_input;
            if (q == 4)
            {
                ss_input << input_name << y << "_Q" << q << "~" << y + 1 << "_Q" << q - 3 << "(" << y << input_format;
            }
            else
            {
                ss_input << input_name << y << "_Q" << q << "-Q" << q + 1 << "(" << y << input_format;
            }

            input_csv = ss_input.str();

            string output_name = "train_result/H2Q/train_";
            string output_format = " Q1)_result.csv";
            stringstream ss_output;
            if (q == 4)
            {
                ss_output << output_name << y << "_Q" << q << "~" << y + 1 << "_Q" << q - 3 << "(" << y << output_format;
            }
            else
            {
                ss_output << output_name << y << "_Q" << q << "-Q" << q + 1 << "(" << y << output_format;
            }

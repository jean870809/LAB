...
int main()
{
    ...
    for (int exp = 0; exp < EXP_TIMES; exp++)
    {
        ...
        for (int t = 0; t < GEN_TIMES; t++) //迭代
        {
            ...
            for (int p = 0; p < P; p++) //更新LB&LW
            {
                ...
                if (globalbest.trend == 0)
                {
                    assign(globalbest, solution[p]);
            ...
            for (int i = 0; i < N; i++)
            {
                if (localbest.choose[i] == 1)
                {
                    r[i] += change;
                }
                if (localworst.choose[i] == 1)
                {
                    r[i] -= change;
                }
            }
    ...
    cout << exp_globalbest.exp << endl;
    cout << exp_globalbest.generation << endl;
    return 0;
}
